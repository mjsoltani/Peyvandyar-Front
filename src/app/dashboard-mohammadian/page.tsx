...
import { Store, ArrowRight, CheckCircle, Loader2, User, Calendar, Plus, ShoppingBag } from "lucide-react";
...
interface ChildStore {
  ...
  mobile: string;
  avatar?: string;
  token?: string; // Store token for future use
  relation_id?: number;
  status?: string;
}
...
export default function MohammadianDashboard() {
      ...
      try {
        ...
        if (data.success && data.user && data.user.basalam_vendor_id) {
          ...
          if (childrenResponse.ok) {
            ...
            // Process children data based on API response structure
            if (childrenData.success && Array.isArray(childrenData.children)) {
              // Get stored child tokens from localStorage
              const storedChildTokens = JSON.parse(localStorage.getItem('child_tokens') || '{}');
              
              // Fetch full info for each child if we have their token
              const childrenPromises = childrenData.children.map(async (child: any) => {
                const childVendorId = child.child_vendor_id;
                const childToken = storedChildTokens[childVendorId];
                ...
                if (childToken) {
                  try {
                    ...
                    if (childInfoResponse.ok) {
                      const childInfo = await childInfoResponse.json();
                      if (childInfo.success && childInfo.data) {
                        return {
                          id: childInfo.data.id,
                          vendor_id: parseInt(childVendorId),
                          vendor_title: childInfo.data.vendor?.title || childInfo.vendor?.title || `غرفه ${childVendorId}`,
                          name: childInfo.data.name,
                          mobile: childInfo.data.mobile,
                          avatar: childInfo.data.avatar?.sm || childInfo.data.avatar?.xs,
                          token: childToken,
                    ...
                    console.error(`Error fetching child ${childVendorId} info:`, err);
                  }
                }
                ...
                return {
                  id: 0,
                  vendor_id: parseInt(childVendorId),
                  vendor_title: `غرفه ${childVendorId}`,
                  name: '',
                  mobile: '',
                  avatar: undefined,
                  relation_id: child.relation_id,
              ...
              const formattedChildren = await Promise.all(childrenPromises);
  ...
  const handleAddChild = async () => {
    ...
    try {
      ...
      // Success - add to list with formatted data
      const childStore: ChildStore = {
        id: childData.data.id,
        vendor_id: childVendorId,
        vendor_title: childData.data.vendor?.title || childData.vendor?.title || 'نامشخص',
        name: childData.data.name,
        mobile: childData.data.mobile,
        avatar: childData.data.avatar?.sm || childData.data.avatar?.xs,
        token: childToken,
        status: 'active'
      };
      
      // Store child token in localStorage for future use
      const storedChildTokens = JSON.parse(localStorage.getItem('child_tokens') || '{}');
      storedChildTokens[childVendorId] = childToken;
      localStorage.setItem('child_tokens', JSON.stringify(storedChildTokens));
      
      setChildStores([...childStores, childStore]);
        ...
        {userData && userData.success && !loading && (
          <div className="max-w-6xl mx-auto">
            ...
            {childStores.length > 0 && (
              <div className="mb-8 bg-white rounded-2xl border border-orange-200 p-8 shadow-lg">
                <h3 className="text-lg font-bold text-slate-800 mb-6 text-center">شبکه غرفه‌های متصل</h3>
                ...
                <div className="relative w-full" style={{ height: '300px' }}>
                  <svg className="w-full h-full" viewBox="0 0 800 300">
                    ...
                    {/* Connection Lines */}
                    {childStores.map((_, index) => {
                      const totalChildren = childStores.length;
                      const spacing = 600 / (totalChildren + 1);
                      const childX = 100 + spacing * (index + 1);
                      
                      return (
                        <g key={index}>
                          <path
                            d={`M 400 80 v 30 q 0 10 ${childX > 400 ? '-' : ''}10 10 h ${Math.abs(childX - 400) - 10} q ${childX > 400 ? '-' : ''}10 0 ${childX > 400 ? '-' : ''}10 10 v 30`}
                            stroke="#f97316"
                            fill="none"
                            strokeWidth="2"
                            strokeDasharray="5 5"
                            opacity="0.5"
                          />
                          <circle cx={childX} cy="160" r="8" fill="#f97316" opacity="0.7">
                    ...
                    {/* Parent Store Badge */}
                    <g>
                      <rect fill="#f97316" x="340" y="50" width="120" height="30" rx="15"></rect>
                      <text x="400" y="70" fill="white" fontSize="14" fontWeight="600" textAnchor="middle">
                        غرفه مادر
                      </text>
                    </g>
                    ...
                    {childStores.map((child, index) => {
                      ...
                      return (
                        <g key={index}>
                          <rect fill="#f97316" x={childX - 60} y="170" width="120" height="30" rx="15" opacity="0.9"></rect>
                          <text x={childX} y="190" fill="white" fontSize="12" fontWeight="500" textAnchor="middle">
                            غرفه {index + 1}
                          </text>
                        </g>
                      );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {childStores.map((child, index) => (
                <div key={index} className="bg-white rounded-2xl border border-orange-200 shadow-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
                  <div className="relative h-40 bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-600 p-4">
                    <div className="absolute top-3 right-3">
                      <div className="bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full">
                  ...
                  <div className="p-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                        <span className="text-slate-600 text-xs">موبایل</span>
                        <span className="text-slate-800 font-bold text-xs" dir="ltr">
                          {child.mobile}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-blue-700 text-xs font-medium">متصل به غرفه مادر</span>
                      </div>
                    </div>
                  </div>
                </div>
