...
import { mixinApi, MixinLink, MixinSyncFields } from "@/lib/api";
...
function extractLinks(response: any): MixinLink[] {
...
function syncToastMessage(result: any, mode: "one" | "all") {
  if (mode === "all" || result?.total != null) {
    return `سینک تمام شد: ${result?.updated ?? 0} به‌روز شد، ${result?.unchanged ?? 0} بدون تغییر، ${result?.failed ?? 0} ناموفق`;
  }
...
export default function MixinLinksPage() {
  const router = useRouter();
  const [links, setLinks] = useState<MixinLink[]>([]);
  ...
  const loadLinks = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const response = await mixinApi.getLinks({ limit: 100, offset: 0 });
      setLinks(extractLinks(response));
    } catch (err: any) {
      const msg = err.message || "خطا در دریافت لینک‌ها";
  ...
  const handleSyncOne = async (link: MixinLink) => {
    try {
      setSyncingOne(link.id);
      const response: any = await mixinApi.sync({
        link_id: link.id,
        fields: fields as MixinSyncFields,
      });
      const data = response.data ?? response;
  ...
  const handleSyncAll = async () => {
    const ok = window.confirm(
      "قیمت/موجودی همه محصولات لینک‌شده از Mixin روی غرفه باسلام به‌روز می‌شود. ادامه می‌دهید؟"
    );
    if (!ok) return;

    try {
      setSyncingAll(true);
      const response: any = await mixinApi.sync({ fields });
      const data = response.data ?? response;
      if (data.success === false) {
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-800">لینک‌های Mixin</h1>
              <p className="text-sm text-slate-500">
                محصولات ایمپورت‌شده و سینک دستی قیمت / موجودی
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            {loading ? (
              ...
              <div className="p-12 text-center text-slate-500">
                هنوز محصولی از Mixin ایمپورت نشده است.
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr className="text-slate-600">
                      ...
                      <th className="text-right p-3 font-medium">قیمت Mixin</th>
                      <th className="text-right p-3 font-medium">قیمت باسلام</th>
                      <th className="text-right p-3 font-medium">موجودی Mixin</th>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-3 whitespace-nowrap">
                            {formatToman(link.last_mixin_price)}
                          </td>
                          <td className="p-3 whitespace-nowrap">
                            {formatToman(link.last_basalam_price)}
                          </td>
                          <td className="p-3 whitespace-nowrap">
                            {link.last_mixin_stock ?? "—"}
                          </td>
                          <td className="p-3 whitespace-nowrap">
