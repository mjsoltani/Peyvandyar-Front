...
const IsoLevelWarp = ({
  ...
  return (
    <div
      ref={containerRef}
      className={cn("absolute inset-0 z-0 overflow-hidden bg-white", className)}
      {...props}
    >
      <canvas ref={canvasRef} className="block w-full h-full" />
      {/* Optional: Vignette overlay for depth */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(255,255,255,0.8)_100%)] opacity-80 pointer-events-none" />
    </div>
  );
};
