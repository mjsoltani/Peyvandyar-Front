...
1. اکانت‌های سوشیال را اضافه می‌کند (`POST /social/accounts` یا در سازگاری قدیمی `PUT /social/setup`)
2. لیست اتصال‌ها را می‌بیند (`GET /social/accounts` یا `GET /social/status`)
3. روی محصول دکمه **انتشار** می‌زند (`publish` با `productId` و در صورت وجود `accountIds`)
...
۱) صفحه تنظیمات سوشیال
   لیست اکانت‌ها + دکمه «افزودن اکانت»
   اگر لیست خالی / NOT_CONFIGURED → فرم افزودن اولین اکانت
   هر کارت: پلتفرم، وضعیت، ویرایش، حذف
...
> غرفه می‌تواند **چند اکانت** سوشیال داشته باشد (حتی چند کانال از یک پلتفرم). UI باید لیست + «افزودن اکانت» باشد، نه یک فرم تکی که اتصال قبلی را جایگزین کند.
...
5. یک غرفه می‌تواند چند اکانت داشته باشد؛ افزودن اکانت جدید نباید بقیه را پاک کند.
